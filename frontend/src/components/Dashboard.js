...
import { Upload, Activity, CheckCircle, AlertTriangle, RefreshCw, ShieldCheck } from 'lucide-react';

import RawSignalView from './RawSignalView';
import SensorMap from './SensorMap';
import PatientView from './PatientView';
import ArtifactDetector from './ArtifactDetector'; // New Integrated Component
  ...
  return (
    <div className="fade-in">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 className="fw-bold mb-0 text-dark">Analysis Results</h2>
      ...
      {!showCleaned ? (
        /* RAW VIEW - Now with Integrated Artifact Detector */
        <div className="row g-4">
          ...
          <div className="col-lg-4">
            <ArtifactDetector data={data} userMode={userMode} />
          </div>
        </div>
      ) : (
        /* CLEANED VIEW */
        <div className="row g-4">
              ...
              <div className="w-100 border rounded-3 bg-light" style={{ overflowX: 'auto', overflowY: 'hidden' }}>
                <div style={{ width: chartWidth, height: 350, backgroundColor: '#fff' }}>
                  ...
                  >
                    ...
                    <YAxis 
                      domain={[-20, 20]} // FIXED RANGE: This stops the auto-magnification
                      tick={{fontSize:12, fill: '#9ca3af'}}
                      width={60}
                    >
                      <Label 
                    ...
                    {/* <Tooltip /> */}
                    <Tooltip 
                      // This function takes the value and formats it to 2 decimal places
          ...
          <div className="col-lg-4 col-12"><SensorMap data={data} /></div>
          ...
          <div className="col-md-4">
            <div className="card border-0 shadow-sm rounded-4 p-4 h-100 bg-white border-bottom border-4 border-warning">
              <div className="d-flex align-items-center gap-3">
                <div className="p-3 bg-warning bg-opacity-10 rounded-3 text-warning"><ShieldCheck size={24}/></div>
                <div>
                  <h6 className="text-muted mb-0 small text-uppercase fw-bold">Signal Health</h6>
                  <h3 className="fw-bold mb-0 text-dark">
                    {data.features?.['T7_DominantFreq'] < 2 ? "Drift Warning" : "Stable"}
                  </h3>
                  <small className="text-muted">
                    {data.features?.['T7_DominantFreq'] < 2 ? "High low-freq noise detected" : "Clean Frequency Output"}
                  </small>
                </div>
              </div>
            </div>
